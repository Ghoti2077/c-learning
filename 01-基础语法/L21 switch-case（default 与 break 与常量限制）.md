...
| 2. `default` 是什么？能省略吗？必须写最后吗？ | 「剩下的所有情况，可以省略，不一定写在最后；但删去 `default`，原本接在 `default` 后面的代码不会进行」 | ⚠️ **前半对、后半表述不清**。准确说法：省掉 `default` = **没匹配到任何 case 时，这条 switch 什么都不做、直接结束**（不是"某段代码不执行"） |
