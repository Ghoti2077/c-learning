...
## 10. 复习题作答与讲评（2026-09-21）

| 题 | 我的答案 | 判定 |
|---|---|---|
| 1. `break` 的作用？不写会怎样？ | 「使代码在这里停止，防止向下穿透；不写会持续向下执行直到遇到另一个 `break`」 | ✅ **完全正确** |
| 2. `default` 是什么？能省略吗？必须写最后吗？ | 「剩下的所有情况，可以省略，不一定写在最后；但删去 `default`，原本接在 `default` 后面的代码不会进行」 | ⚠️→✅ **他提了反问，最后说清了**：观察没错（删了标签后那句 `printf` 确实不执行），但**原因**不是"删了 default 就失效"，而是**那句 `printf` 没有标签领着、谁都跳不到它**（不可达代码）。把 `default:` 标签加回来、`printf` 一字不改 → 输入 5 立刻就能打印。详见第 11 节 |
| 3. 哪些能当 `case` 标签？ | 「`case 1`、`case 2+3`」 | ✅ **答对**（漏了"为什么另外两个不行"）：`case a:`（变量）、`case ONE:`（`const int`）都报 `C2051 / case label does not reduce to an integer constant`——**编译期常量才行** |
| 4. 故意不写 `break` 不接 `case 2:` 是 bug 还是技巧？ | 「技巧，可以达到限制多种条件的效果」 | ⚠️ **半对**。是技巧没错，但它叫**穿透（fall-through）**，用途是**让多个 case 共用同一段代码**（`case 1: case 2:` 都打印"你好"）——"限制多种条件"这个说法不准确 |

**讲评**：第 1、3 题干净利落；第 2、4 题**意思摸到了但说法不准**——"表述要精确"和"答题要答全"是同一件事，继续盯。

## 11. 我的反问：删了 `default` 之后那句 `printf` 为什么不执行？（顺便讲清"编译期常量"）

**我的反问**：「去掉 `default` 后，我输入了 5，后面的 `printf("啊，神魔啊")` 却并没有执行——这难道不是我说的'某段代码不执行'？」

### 先承认：**我的观察没错**，AI 上一轮的说法让我误解了

![[L21-追问 删掉default标签后的运行结果.png]]

AI 把代码改成这样（删掉 `default:` 这个**标签**，`printf` 和 `break` 都留着）：
...
case 4: printf("再见"); break;
    printf("[孤儿语句] 啊神魔啊");     /* ← 没有标签领着它 */
    break;
}
```

**AI 实测**：输入 5 → **什么都不打印**（和我在 VS 里看到的一模一样 ✅）。

所以"那句 `printf` 没执行"是**事实**。要掰的不是现象，是**原因**。

### 决定性实验：只把 `default:` 标签加回来，`printf` **一个字都不改**

| 版本 | 输入 5 | 输入 4 |
|---|---|---|
| **没有** `default:` 标签 | 什么都不打印 | `再见` |
| **加回** `default:` 标签 | **打印"啊神魔啊"** ✅ | `再见` |

**同一个 `printf`、同一行、同一个位置**，只差一个标签，行为完全不同。

> **结论**：决定它能不能执行的是——**有没有标签指过来**。
> 不是"删了 `default` 导致它失效"，而是**它成了没有入口的代码**（教科书叫 **不可达代码 / unreachable code**）。

### 为什么？因为 `switch` 是"跳转"，不是"顺序往下读"

- 进 `switch` 时先算 `type` 的值，然后**跳到对应标签那一行**开始执行；
- 输入 5：4 个 `case` 都不匹配、又没有 `default` → **直接跳过整个 `switch` 块**，从 `switch { }` **后面**那句继续；
- 所以那块 `printf` 永远不会有"跳进来"的机会。

**证据：VS 的编译器自己也这么说。** 把警告等级调到 **/W4**，编译上面那段会给出：

```
warning C4702: 无法访问的代码
```

（VS 项目默认是 **/W3** → **不提醒**，所以我那边 0 警告是正常的；想看这种提示就把警告等级提到 /W4：项目属性 → C/C++ → 常规 → 警告等级。）

### `default:` 放哪儿都行（它是**标签**，不是"尾段"）

- 实测把 `default:` 放**中间**：`type=2` 照样进 `case 2`，`type=9` 才进 default；
- **实测**把 `default:` 放**最前面**：输入 5 → 打印 `other`（跳进去了）、输入 1 → `one`、输入 2 → `two`——**这次有标签可跳**，所以没匹配的输入一样会执行那块代码。

### 编译期常量到底是什么

| | 含义 | 例子 |
|---|---|---|
| **编译期常量**（constant expression） | **编译器在编译时（程序还没跑）就能算出确切值**的式子 | `1`、`'a'`、`2 + 3`、`10 - 4`、`sizeof(char)` |
| **不是**编译期常量 | 值要**等程序跑起来才知道** | 变量 `a`、**`const int ONE`（C 里仍是变量）**、`scanf` 读进来的值、函数返回值 |

三个关键点：

1. `case` 标签**必须是编译期常量**——因为编译器要拿这些值**提前建一张"跳转表"**（`switch` 之所以比一串 `if-else` 快，就快在这儿）。
2. `const` 的准确含义是"**只读**"：**运行时不能改**。这跟"**编译期就知道值**"是**两件不同的事**。C 里 `const int ONE = 1;` 的初值虽然写在代码里，标准仍然把它当**变量** → `case ONE:` 报 `C2051`。（**C++ 才额外允许** `const int` 当常量表达式。）
3. 我实测过 `case sizeof(char):` —— **合法**（编译期算出 1，命中正确）。所以 `sizeof` 这种也算编译期常量。

> **一句话记住**：
> **`case` 要的是"编译期就能算出来的整数"；`const` 给的是"运行期不能改的变量"。两码事。**
